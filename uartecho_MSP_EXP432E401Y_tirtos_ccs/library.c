...
// When HWI happens, post an SWI and get it to lower priority
void Timer0Callback(Timer_Handle myTimerHandle, int_fast16_t status)
{
    Glo.callbacks[0].hwiFlag = false;
    Swi_post(Glo.bios.timer0Swi);
}


// gate is used to prevent the callback stream from being read and being written at the same time
void SWITimer0(UArg arg0, UArg arg1)
{
    uint32_t gateKey;
    gateKey = GateSwi_enter(Glo.bios.callbackGate);
    if(Glo.callbacks[0].count != 0)
    {
        if (Glo.callbacks[0].count > 0)
        {
            Glo.callbacks[0].count--;
        }
        if (Glo.callbacks[0].HWI_flag == false)
        {
            AddPayload(Glo.callbacks[0].payload); // Add callback payload to payload queue
        }
        Glo.adcBufCtrl.callback0_count++;
    }
...
void TaskPayload()
{
    ...
    while(1)
    {
        Semaphore_pend(Glo.bios.payloadSemaphore, BIOS_WAIT_FOREVER);
        index = Glo.payloadQ.payloadReading;
        payload = Glo.payloadQ.payloads[index];
        parseMessage(payload);

        gateKey = GateSwi_enter(Glo.bios.payloadWriteGate);
        payloadNext = index + 1;
        if(payloadNext >= NUMPAYLOADS)
            payloadNext = 0;
        Glo.payloadQ.payloadReading = payloadNext;
...
void TaskUartRead()
{
        ...
        while (1)
        {
            ...
            else if (inputChar == '\n' || inputChar == '\r') // Enter pressed
            {
                ...
                int payloadIndex = Glo.payloadQ.payloadWriting;
                strcpy(Glo.payloadQ.payloads[payloadIndex], inputLine);

                if(Glo.payloadQ.payloadWriting == NUMPAYLOADS - 1)
                {
                    Glo.payloadQ.payloadWriting = 0;
                }
                else
                {
                    Glo.payloadQ.payloadWriting++;
                }
            }
...
void TaskParseMessage()
{
    char        inputLine[MAXLEN + 1];
    inputLine[0] = 0;
    while(1)
    {
        int payloadIndex = Glo.payloadQ.payloadReading;
        strcpy(inputLine, Glo.payloadQ[payloadIndex]);
        if (Glo.payloadQ.payloadReading == NUMPAYLOADS - 1)
        {
            Glo.payloadQ.payloadReading = 0;
        }
        else
        {
            Glo.payloadQ.payloadReading++;
        }
...
void MsgAddByte(char *input)
{
    ...
    if(*input == '\r' || *input == '\n')
    {
        if (Glo.uartInQ.messages[Glo.uartInQ.msgWriting].msgCount > 0)
            Glo.MsgQ.messages[Glo.uartInQ.msgWriting].msgBuf[Glo.MsgQ.messages[Glo.MsgQ.msgWriting].msgCount] = 0;
        gateKey = Gate
    }
}
